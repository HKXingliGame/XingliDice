...
Dice = function (game, x, y) {
    ...
    Phaser.Sprite.call(this, this.game, x, y, 'dice');

    this.tween;
    this.anim;
    this.blurX = this.game.add.filter("BlurX");  // Blur filters taken from
    this.blurY = this.game.add.filter("BlurY");  // Filters -> blur example
    ...
    var frames = [];
    for (i = 0; i < 15; i++) {
        frames[i] = this.game.rnd.pick([0, 1, 2, 4, 5, 6]);
    }
